...
function ProfileSettings({ profile, save, go }: { profile: Profile; save: (profile: Profile) => void; go: (screen: Screen) => void }) {
  ...
  return <main className="app-screen settings-screen screen-enter">
    <header className="simple-header"><button onClick={() => go("profile")} aria-label="返回我的資料">←</button><span>資料設定</span><i>一次完成</i></header>
    ...
    <section className="settings-section"><div className="settings-section-title"><span>01</span><div><h2>身體與目標</h2><p>用來估算每日範圍與份量方向。</p></div></div><div className="form-grid"><label>年齡<input inputMode="numeric" value={draft.age} onChange={event => update("age", event.target.value)} /><span>歲</span></label><label>身高<input inputMode="numeric" value={draft.height} onChange={event => update("height", event.target.value)} /><span>cm</span></label><label>體重<input inputMode="numeric" value={draft.weight} onChange={event => update("weight", event.target.value)} /><span>kg</span></label><label>性別<select value={draft.gender} onChange={event => update("gender", event.target.value)}><option>女性</option><option>男性</option><option>其他</option></select></label></div><div className="field-block"><span className="field-title">目前目標</span><div className="chip-row wrap">{["減脂", "維持", "增肌", "均衡飲食"].map(value => <ToggleChip key={value} value={value} selected={draft.goal === value} onClick={() => update("goal", value)} />)}</div></div><div className="field-block"><span className="field-title">運動量</span><div className="chip-row wrap">{["幾乎不運動", "每週 1 天", "每週 2–3 天", "每週 4 天以上"].map(value => <ToggleChip key={value} value={value} selected={draft.activity === value} onClick={() => update("activity", value)} />)}</div></div></section>
    <section className="settings-section"><div className="settings-section-title"><span>02</span><div><h2>飲食偏好</h2><p>硬性限制會排除；口味只影響排序。</p></div></div><div className="field-block"><span className="field-title">過敏／宗教／醫療限制</span><div className="chip-row wrap">{["不吃牛", "無乳製品", "堅果過敏", "素食"].map(value => <ToggleChip key={value} value={value} selected={draft.exclusions.includes(value)} onClick={() => toggle("exclusions", value)} />)}</div></div><div className="field-block"><span className="field-title">口味與排序偏好</span><div className="chip-row wrap">{["少辣", "低糖", "預算 150 內"].map(value => <ToggleChip key={value} value={value} selected={draft.preferences.includes(value)} onClick={() => toggle("preferences", value)} />)}</div></div></section>
    ...
    <div className="settings-actions"><button className="primary-btn" disabled={invalid} onClick={() => save(draft)}>儲存所有設定 <span>→</span></button><button className="secondary-btn" onClick={() => go("profile")}>取消</button></div>
  </main>;
}
...
export default function HomePage() {
  ...
  else if (screen === "settings") content = <ProfileSettings profile={profile} save={next => { setProfile(next); go("profile"); }} go={go} />;
